...
Route::get("verify/{id}", [UserController::class, "verifyUser"])->middleware(
    "auth:api",
);
...
Route::middleware("auth:api")
    ...
    ->group(function () {
        ...
        Route::prefix("services")->group(function () {
            Route::prefix("bbps")
                ->controller(BbpsFlowController::class)
                ->group(function () {
...
Route::prefix("admin")
    ...
    ->group(function () {
        ...
        Route::group(["prefix" => "manage-access"], function () {
            ...
            Route::put("sync-user-permissions/{user}", [
                AdminController::class,
                "updateUserPermission",
            ]);
            Route::put("sync-role-permissions/{role}", [
                AdminController::class,
                "updateRolePermission",
            ]);
            Route::get("role-permissions/{role}", [
                AdminController::class,
                "rolePermissions",
            ]);
        ...
        Route::group(["prefix" => "manage-user"], function () {
            ...
            Route::put("address/{user_id}", [
                AdminUserController::class,
                "address",
            ]);
            Route::get("address/{user_id}", [
                AdminUserController::class,
                "getAddress",
            ]);
            ...
            Route::put("send-credentials/{user}", [
                AdminUserController::class,
                "sendCredential",
            ]);
        ...
        Route::prefix("commissions")->group(function () {
            Route::get("get-commission/{id}", [
                CommissionController::class,
                "getCommission",
            ]);
            Route::post("create-commission", [
                CommissionController::class,
                "createCommission",
            ]);
            Route::put("update-commission/{id}", [
                CommissionController::class,
                "updateCommission",
            ]);
            Route::delete("delete-commission/{id}", [
                CommissionController::class,
                "deleteCommission",
            ]);
        });
...
Route::prefix("callback/payout")
    ...
    ->group(function () {
        ...
        Route::any("zaypay", "zaypay");
    });

Route::get("fail-transaction/{transction_id}", function (
    string $transction_id,
) {
    $payout = Payout::where("reference_id", $transction_id)->first();
    if (
        $payout &&
        in_array($payout->status, ["pending", "processing", "initiated"])
    ) {
